  ...
  async function updateRealTimeData() {
    ...
    try {
      ...
      setTimeout(() => {
          ...
          setTimeout(() => tables.forEach(table => table.classList.remove('price-updated')), 1000);
      }, 500);
...
function recalculateSummaryMetrics() {
  const totalStockValue = portfolioData.holdings.reduce((sum, h) => sum + (h.shares * h.currentPrice), 0);
  const totalCostBasis = portfolioData.holdings.reduce((sum, h) => sum + (h.shares * h.avgPrice), 0);
  ...
  async function loadAllData() {
    ...
    // const performanceData = await fetchData("performance");
    // if (performanceData) {
    //   portfolioData.performance = performanceData;
    // }
  
    // // Fetch allocation data
    // const allocationData = await fetchData("allocation");
    // if (allocationData) {
    //   portfolioData.allocation = allocationData;
    // }
  
    // // Fetch holdings data
    // const holdingsData = await fetchData("holdings");
    // if (holdingsData) {
    //   portfolioData.holdings = holdingsData;
    //   filteredHoldings = [...holdingsData]; // Initialize filtered holdings
    // }

    // // Fetch default search data (popular stocks)
    // const searchData = await fetchData("search");
    // if (searchData) {
    //   portfolioData.searchResults = searchData;
    //   filteredResults = [...searchData]; // Initialize filtered results
    // }
    
    // const summaryData = await fetchData("summary");
    // if (summaryData) {
    //   portfolioData.summary = summaryData;
    // }
    // const transactionsData = await fetchData("transactions");
    // if (transactionsData) {
    //   portfolioData.transactions = transactionsData;
    // }
    ...
    const [performance, allocation, holdings, search, summary] = await Promise.all([
    ...
    portfolioData.allocation = allocation || { labels: [], values: [], colors: [] };
...
async function refreshDashboardPostTransaction(newSummary) {
  ...
  function updateMetrics() {
    const { totalPortfolioValue, totalGainLoss, cashBalance, totalHoldings } = portfolioData.summary;

    document.querySelector(".metric-card:nth-child(1) .metric-value").textContent = `$${(totalPortfolioValue || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    document.querySelector(".metric-card:nth-child(2) .metric-value").textContent = `${(totalGainLoss || 0) >= 0 ? "+" : ""}$${Math.abs(totalGainLoss || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    document.querySelector(".metric-card:nth-child(3) .metric-value").textContent = `$${(cashBalance || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    document.querySelector(".metric-card:nth-child(4) .metric-value").textContent = totalHoldings || 0;
  }
  ...
  //   const filteredResults = portfolioData.searchResults.filter(
  //     (stock) =>
  ...
  //             <td class="${
  //               stock.change.startsWith("+") ? "trend-positive" : "trend-negative"
  //             }">${stock.change}</td>
  ...
  async function fetchAndPopulateSearch(query) {
      try {
          // Show loading state
          const tbody = document.getElementById('searchTableBody');
          tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: #666;">Searching...</td></tr>';
  ...
  function populateSearchTable(results) {
      ...
      const paginatedResults = getPaginatedData(results, currentPage, recordsPerPage);
      ...
      paginatedResults.forEach(stock => {
          ...
          row.innerHTML = `
              ...
              <td><button class="btn btn-primary" onclick="openAddModal('${escapedSymbol}', '${escapedName}', ${stock.price || 0})">Add</button></td>
          `;
          tbody.appendChild(row);
      });
  ...
  function setupSearch() {
    ...
    // Keeping it for backward compatibility but it's not needed
    console.log("Search setup completed - using handleSearchInput for real-time search");
  }
...
// --- Sell Modal Logic ---
  ...
  async function confirmSell() {
      ...
      const response = await postData("stock/sell", { symbol: symbolToSell, shares: shares });
      if (response) {
          alert(response.message);
          closeSellModal();
...
function openAddModal(symbol, name, price) {
    ...
    modalDescription.textContent = `Enter the number of shares to buy for ${name} ($${price.toFixed(2)} per share)`;
    quantityInput.value = 1;
    quantityInput.min = 1;
...
async function confirmBuy() {
    ...
    isTransactionInProgress = true;
    const response = await postData("stock/add", { symbol: symbolToBuy, shares: shares });
    if (response) {
        alert(response.message);
        closeBuyModal();
        await refreshDashboardPostTransaction(response.summary);
